...
async function redirect(req, res, next) {
  ...
  // 7. Check for splash page
  const { LinkSplashPages } = require("../models");
  if (link.has_splash_page) {
    const splashPage = await LinkSplashPages.query()
      .findOne("link_id", link.uuid)
      .where("is_active", true);
      
    if (splashPage) {
      // Redirect to splash page instead of direct target
      return res.redirect(`/s/${link.uuid}/splash`);
    }
  }
