...
async function homepage(req, res) {
  res.render("homepage", {
    title: "maerz.click - Modern URL Shortener",
  });
}
...
async function splashPage(req, res) {
  ...
  try {
    ...
    const splashPage = await LinkSplashPages.query()
      .findOne("link_id", req.params.id)
      .where("is_active", true);

    if (!splashPage) {
      // No splash page, redirect directly
      return res.redirect(link.target);
    }

    res.render("splash-page", {
      title: splashPage.branding_config?.title || "Redirecting...",
      splashPage,
      link,
      layout: false // No layout for splash pages
    });
  } catch (error) {
    console.error("Error loading splash page:", error);
    res.status(500).render("error", {
      title: "Error loading splash page"
    });
  }
}
